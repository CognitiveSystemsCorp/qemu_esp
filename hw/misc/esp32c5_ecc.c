...
#define ECC_MULT_PX_MEM      0x130
#define ECC_MULT_PY_MEM      0x160
#define ECC_MULT_QX_MEM      0x190
#define ECC_MULT_QY_MEM      0x1c0
#define ECC_MULT_QZ_MEM      0x1f0
#define ECC_MULT_MEM_SIZE    48

#define ECC_MULT_START_BIT   (1 << 0)
#define ECC_MULT_CALC_DONE_INT (1 << 0)
#define ECC_MULT_KEY_LENGTH_SHIFT 2
#define ECC_MULT_KEY_LENGTH_MASK  (3 << ECC_MULT_KEY_LENGTH_SHIFT)
#define ECC_MULT_WORK_MODE_SHIFT 5
#define ECC_MULT_WORK_MODE_MASK  (15 << ECC_MULT_WORK_MODE_SHIFT)
#define ECC_MULT_VERIFY_RESULT_BIT (1 << 29)

#define ECC_MODE_POINT_MUL 0
#define ECC_MODE_VERIFY 2
#define ECC_MODE_VERIFY_THEN_POINT_MUL 3

static void esp32c5_ecc_update_irq(ESP32C5EccState *s)
{
...
#ifdef CONFIG_GCRYPT
static bool esp32c5_ecc_do_math(ESP32C5EccState *s)
{
    gcry_ctx_t ctx = NULL;
    gcry_mpi_t px = NULL, py = NULL, scalar = NULL, pz = NULL;
    gcry_mpi_t qx = NULL, qy = NULL;
    gcry_mpi_point_t p = NULL, q = NULL;
    unsigned curve = (s->conf & ECC_MULT_KEY_LENGTH_MASK) >>
                     ECC_MULT_KEY_LENGTH_SHIFT;
    const char *curve_name = curve == 1 ? "NIST P-256" : "NIST P-192";
    ...
    if (gcry_mpi_ec_new(&ctx, NULL, curve_name)) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "esp32c5.ecc: cannot create %s context\n", curve_name);
        return false;
    }
    ...
    if (mode != ECC_MODE_POINT_MUL &&
        ...
        qemu_log_mask(LOG_GUEST_ERROR,
                      "esp32c5.ecc: unsupported work mode %u\n", mode);
        goto out;
    }
...
{
    qemu_log_mask(LOG_GUEST_ERROR,
                  "esp32c5.ecc: QEMU was built without libgcrypt\n");
    return false;
}
#endif

static uint64_t esp32c5_ecc_read(void *opaque, hwaddr addr, unsigned size)
{
    ESP32C5EccState *s = ESP32C5_ECC(opaque);

    if (addr >= ECC_MULT_K_MEM && addr < ECC_MULT_K_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->k_mem[addr - ECC_MULT_K_MEM], size);
        return val;
    }
    if (addr >= ECC_MULT_PX_MEM && addr < ECC_MULT_PX_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->px_mem[addr - ECC_MULT_PX_MEM], size);
        return val;
    }
    if (addr >= ECC_MULT_PY_MEM && addr < ECC_MULT_PY_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->py_mem[addr - ECC_MULT_PY_MEM], size);
        return val;
    }
    if (addr >= ECC_MULT_QX_MEM && addr < ECC_MULT_QX_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->qx_mem[addr - ECC_MULT_QX_MEM], size);
        return val;
    }
    if (addr >= ECC_MULT_QY_MEM && addr < ECC_MULT_QY_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->qy_mem[addr - ECC_MULT_QY_MEM], size);
        return val;
    }
    if (addr >= ECC_MULT_QZ_MEM && addr < ECC_MULT_QZ_MEM + ECC_MULT_MEM_SIZE) {
        uint32_t val;
        memcpy(&val, &s->qz_mem[addr - ECC_MULT_QZ_MEM], size);
        return val;
    }
...
static void esp32c5_ecc_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    ESP32C5EccState *s = ESP32C5_ECC(opaque);
    ...
    if (addr >= ECC_MULT_QX_MEM && addr < ECC_MULT_QX_MEM + ECC_MULT_MEM_SIZE) {
        memcpy(&s->qx_mem[addr - ECC_MULT_QX_MEM], &val, size);
        return;
    }
    if (addr >= ECC_MULT_QY_MEM && addr < ECC_MULT_QY_MEM + ECC_MULT_MEM_SIZE) {
        memcpy(&s->qy_mem[addr - ECC_MULT_QY_MEM], &val, size);
        return;
    }
    if (addr >= ECC_MULT_QZ_MEM && addr < ECC_MULT_QZ_MEM + ECC_MULT_MEM_SIZE) {
        memcpy(&s->qz_mem[addr - ECC_MULT_QZ_MEM], &val, size);
        return;
    }
    ...
    case ECC_MULT_CONF_REG:
        ...
        if (val & ECC_MULT_START_BIT) {
            if (!esp32c5_ecc_do_math(s)) {
                qemu_log_mask(LOG_GUEST_ERROR,
                              "esp32c5.ecc: ECC operation failed\n");
            }
...
static const MemoryRegionOps esp32c5_ecc_ops = {
    .read = esp32c5_ecc_read,
    .write = esp32c5_ecc_write,
...
static void esp32c5_ecc_realize(DeviceState *dev, Error **errp)
{
    ESP32C5EccState *s = ESP32C5_ECC(dev);
    SysBusDevice *sbd = SYS_BUS_DEVICE(dev);

    memory_region_init_io(&s->iomem, OBJECT(s), &esp32c5_ecc_ops, s,
                          TYPE_ESP32C5_ECC, 0x1000);
    sysbus_init_mmio(sbd, &s->iomem);
    sysbus_init_irq(sbd, &s->irq);
}

static void esp32c5_ecc_reset(DeviceState *dev)
{
    ESP32C5EccState *s = ESP32C5_ECC(dev);
    ...
    memset(s->qx_mem, 0, sizeof(s->qx_mem));
    memset(s->qy_mem, 0, sizeof(s->qy_mem));
    memset(s->qz_mem, 0, sizeof(s->qz_mem));
}

static void esp32c5_ecc_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->realize = esp32c5_ecc_realize;
    device_class_set_legacy_reset(dc, esp32c5_ecc_reset);
}

static const TypeInfo esp32c5_ecc_info = {
    .name          = TYPE_ESP32C5_ECC,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(ESP32C5EccState),
    .class_init    = esp32c5_ecc_class_init,
};

static void esp32c5_ecc_register_types(void)
{
    type_register_static(&esp32c5_ecc_info);
}

type_init(esp32c5_ecc_register_types)
