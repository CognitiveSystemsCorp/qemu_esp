...
static uint64_t esp32_phya_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    /*
     * On ESP32-C6 offset zero of the RF/BB block at 0x600ad000 is a
     * free-running microsecond counter.  The closed PHY library uses it to
     * bound calibration waits (for example wait_i2c_sdm_stable()).
     */
    if (addr == 0) {
        r = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) / 1000;
    }

    /* Power-detector calibration state machine idle/done state. */
    if (addr == 0x814) {
        r |= 7 << 14;
    }
    ...
    /* RF calibration command completion. */
    if (addr == 0x4a0) {
        r |= BIT(16);
    }
...
static void esp32_phya_write(void *opaque, hwaddr addr, uint64_t value,
  ...
  /* TX DC calibration starts with bit 0 and completes with status bit 22. */
  if (addr == 0x418 && (value & BIT(0))) {
      s->mem[addr / 4] |= BIT(22);
  }
}
