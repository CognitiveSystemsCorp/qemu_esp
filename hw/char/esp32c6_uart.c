...
static uint64_t esp32c6_uart_read(void *opaque, hwaddr addr,
                                  unsigned int size)
{
    ESP32C6UARTState *s = ESP32C6_UART(opaque);
    ESP32C3UARTClass *parent_class = ESP32C3_UART_GET_CLASS(opaque);
    ESP32UARTState *uart = &s->parent.parent;
    uint32_t value = 0;

    switch (addr) {
    case A_ESP32C6_UART_CONF1:
        return s->parent.conf1;

    case A_ESP32C6_UART_MEM_CONF:
    case A_ESP32C6_UART_TOUT_CONF:
        return uart->reg[addr / sizeof(uint32_t)];

    case A_ESP32C6_UART_MEM_RX_STATUS: {
        uint32_t fifo_size = fifo8_num_used(&uart->rx_fifo);

        /* Keep RD_ADDR at zero and expose the current occupancy as WR_ADDR. */
        value = FIELD_DP32(value, ESP32C6_UART_MEM_RX_STATUS, RD_ADDR, 0);
        value = FIELD_DP32(value, ESP32C6_UART_MEM_RX_STATUS, WR_ADDR,
...
static void esp32c6_uart_write(void *opaque, hwaddr addr, uint64_t value,
                               unsigned int size)
{
    ESP32C6UARTState *s = ESP32C6_UART(opaque);
    ESP32C3UARTClass *parent_class = ESP32C3_UART_GET_CLASS(opaque);
    ...
    switch (addr) {
    case A_ESP32C6_UART_CONF0: {
        uint32_t autobaud = FIELD_EX32(value, ESP32C6_UART_CONF0,
                                       AUTOBAUD_EN);

        parent_class->parent_uart_write(opaque, A_UART_AUTOBAUD, autobaud,
                                        sizeof(uint32_t));
        if (FIELD_EX32(value, ESP32C6_UART_CONF0, RXFIFO_RST)) {
            fifo8_reset(&uart->rx_fifo);
        }
        if (FIELD_EX32(value, ESP32C6_UART_CONF0, TXFIFO_RST)) {
    ...
    case A_ESP32C6_UART_CONF1:
        ...
        uart->rx_full_threshold = FIELD_EX32(value, ESP32C6_UART_CONF1,
                                             RXFIFO_FULL_THRHD);
        uart->tx_empty_threshold = FIELD_EX32(value, ESP32C6_UART_CONF1,
                                              TXFIFO_EMPTY_THRHD);
    ...
    case A_ESP32C6_UART_TOUT_CONF:
        ...
        uart->rx_tout_ena = FIELD_EX32(value, ESP32C6_UART_TOUT_CONF,
                                       RX_TOUT_EN);
        uart->rx_tout_thres = FIELD_EX32(value, ESP32C6_UART_TOUT_CONF,
                                         RX_TOUT_THRHD);
...
static void esp32c6_uart_class_init(ObjectClass *klass, void *data)
{
    ...
    uart_class->uart_read = esp32c6_uart_read;
    uart_class->uart_write = esp32c6_uart_write;
}
